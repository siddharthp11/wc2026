...
- **`index.ts`**: Main CLI entry point. Handles command routing (`group` and `team` commands) and data file loading
- **`types.ts`**: TypeScript type definitions for all data structures (Teams, Groups, Players, etc.)
- **`2026/`**: Data directory containing JSON files sourced from [openfootball/worldcup.json](https://github.com/openfootball/worldcup.json)
